...
function makerow(member) {
	...
	var mno = member.getElementsByTagName("mno")[0].firstChild.data
	...
	var email = member.getElementsByTagName("email")[0].firstChild.data;
	var tel = member.getElementsByTagName("tel")[0].firstChild.data;
	...
	var cel1 = makecell(mno,"mno");
	var cel2 = makecell(userid,"id");
	var cel3 = makecell(name,"name");
	var cel4 = makecell(email,"email");
	var cel5 = makecell(tel,"tel");
...
function makecell(data,tag) {
	...
	if(tag == "status") {
		...
		if(txtNode.nodeValue == '회원'){
			option.setAttribute("selected", "selected");
		}else if(txtNode.nodeValue == '탈퇴회원'){
			option1.setAttribute("selected", "selected");
		}else if(txtNode.nodeValue == '관리자'){
			option2.setAttribute("selected", "selected");
		}
		select.appendChild(option);
		option.setAttribute("value", "1");		
		option.appendChild(document.createTextNode("회원"));
		select.appendChild(option1);
		option1.setAttribute("value", "0");
		option1.appendChild(document.createTextNode("탈퇴회원"));
		select.appendChild(option2);
		option2.setAttribute("value", "9");
		option2.appendChild(document.createTextNode("관리자"));		
...
function changetag(obj) {
		...
		var td = tr.children();
		var mno = td.eq(0).text();
		var tag = obj.value;		
		var params = "act=changetag&num="+mno+"&no=mno&tag="+encodeURL(tag);
...
function statusChange(obj){
	   ...
	   var mno = td.eq(0).text();
	   var stat = obj.value;
	   var params = "act=changestat&num="+mno+"&stat="+stat+"&no=mno";
	   sendRequest("<%=root%>/admin", params, memberlist, "GET");
	}
		...
		<div class="row">
			<div class="col-lg-12 pt-4">
				<div class="panel panel-default">
					...
					<option value="m_tag">헤쉬테그</option>
					</select>
